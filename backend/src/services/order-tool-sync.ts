...
async function lookupShippingAddress(trackingNumber: string, shippingMethod?: string): Promise<string> {
...
async function resolveShippingAddress(item: OrderToolShippedItem): Promise<string> {
  ...
  return lookupShippingAddress(item.tracking_number, item.shipping_method)
}
